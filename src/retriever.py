...
_SINGLE_QUERY_PROMPT = (
    "You are a clinical librarian searching PubMed for a complex patient case.\n"
    "Rewrite the admission note as a focused clinical question suitable for PubMed retrieval.\n\n"
    "Use the PICO framework where applicable:\n"
    "- Patient/Problem: key diagnosis or condition\n"
    "- Intervention/Finding: key procedure, lab, or finding\n"
    "- Comparison: if relevant\n"
    "- Outcome: complication, diagnosis, or prognosis\n\n"
    "Rules:\n"
    "- Formulate as a question (e.g., 'What are the complications of...', 'How is X diagnosed in patients with Y?')\n"
    "- Focus on the most clinically complex or unusual aspect.\n"
...
_MULTI_QUERY_PROMPT = (
    "You are an expert medical coder. Rewrite this admission note into exactly {n} "
    "PubMed search queries optimized for finding ICD-10 coding guidelines.\n\n"
    "Generate queries targeting:\n"
    "1. The most SPECIFIC and RARE ICD codes applicable to this case "
    "(focus on 4th and 5th digit specificity, e.g., not just 'diabetes' but "
    "'diabetes with specific complication').\n"
    "2. The primary diagnosis with its most specific clinical manifestations.\n"
    "3. Secondary diagnoses and complications that would each require a separate ICD code.\n\n"
    "Rules:\n"
    "- Each query should target a DIFFERENT set of potential ICD codes.\n"
    "- Prioritize specificity over breadth.\n"
    "- Use clinical terms that appear in ICD-10 descriptions.\n"
